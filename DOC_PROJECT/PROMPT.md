...
16. **`REAL_APIGEE_VALIDATION.md`** — procédure de validation réelle Hello World sans exposer de credential
17. **`REAL_APIGEE_HELLOWORLD_REPORT.md`** — rapport de la première validation réelle et des corrections de mapping
18. **`PACKAGING.md`** — stratégie de build et de distribution (CLI seul + GUI+CLI ensemble)
19. **`GCP_SETUP.md`** — comment l'environnement Google Cloud de test est/sera provisionné
20. **`schemas/template.schema.json`** et **`schemas/template.example.json`** — format de données central du projet
